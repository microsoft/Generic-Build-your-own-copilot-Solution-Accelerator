...
def create_or_update_search_index(
    ...
    if response.status_code == 201:
        ...
        raise Exception(
            f"Failed to create search index. Error: {response.text}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, help="Path to config file containing settings for data preparation")
    ...
    parser.add_argument("--azure-openai-key", type=str,
                        help="Key for the (Azure) OpenAI API.")
