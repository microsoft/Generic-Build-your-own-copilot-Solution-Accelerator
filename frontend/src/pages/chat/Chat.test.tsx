...
jest.mock('react-router-dom', () => ({
  ...
  useLocation: jest.fn(),
  useNavigate: jest.fn(), // Initialize useNavigate as a mock function
}));
...
describe('Chat Component', () => {
  ...
  beforeEach(() => {
    ...
    (ReactRouterDom.useLocation as jest.Mock).mockImplementation(() => ({
      pathname: '/my-path',
      search: '',
      hash: '',
      state: null,
    ...
    // Mock useNavigate to be a jest function we can assert on
    (ReactRouterDom.useNavigate as jest.Mock).mockImplementation(() => jest.fn());
  ...
  test('Should show Auth not configured when userList length zero', async () => {
    ...
    renderWithContext(<Chat type={ChatType.Browse} />, mockState)
    await waitFor(() => {
  ...
  test('Should not show Auth not configured when auth_enabled is false', async () => {
    ...
    renderWithContext(<Chat type={ChatType.Browse} />, tempMockState)
    await waitFor(() => {
      expect(screen.queryByText('AuthNotConfigure Mock')).not.toBeInTheDocument()
    })
  })
  ...
  test('Should handle stop generating event when is inprogress', async () => {
    userEvent.setup()
  ...
  test('should handle historyGenerate API failure correctly', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(
          /There was an error generating a response. Template history can't be saved at this time. Please try again. If the problem persists, please contact the site administrator/i
        )
      ).toBeInTheDocument()
    })
  })
  ...
  test('Question input on click handler when response rendering', async () => {
    userEvent.setup()
  ...
  test('Should handle historyGenerate API returns empty object or null', async () => {
    ...
    await waitFor(async () => {
      expect(
        screen.getByText(/An error occurred. Please try again. If the problem persists, please contact the site administrator./i)
      ).toBeInTheDocument()
    })
  })
...
/*
  test.only('Should handle onShowCitation method when citation button click', async () => {
    userEvent.setup()
